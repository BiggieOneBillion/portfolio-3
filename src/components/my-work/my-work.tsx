...
import { InfiniteMovingTech } from "../global/infinte-moving-tech";
import { li } from "framer-motion/client";

const imageUrl =
  "https://images.unsplash.com/photo-1663765970236-f2acfde22237?q=80&w=3542&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D";

const categories = ["External Api Integration", "Utilities", "Backend Services"];
    ...
    <section className="min-h-screen h-full pt-[10rem] space-y-5 px-3 md:px-0">
      ...
      <div className="max-w-4xl mx-auto flex flex-col gap-10 overflow-hidden">
        ...
        {dataArr[count].map((datum) => (
          <article className="flex flex-col items-center md:flex-row md:items-start gap-10 px-3 md:px-0 bordery" key={v4()}>
            {/* image container */}
            <DirectionAwareHoverCard imageUrl={datum.imgUrl ? datum.imgUrl : imageUrl}>
            ...
            <div className="flex-1 flex flex-col gap-8">
              ...
              <div className="flex items-center gap-3">
                <Link
                  ...
                  to={datum.links.github}
                >
                  Github Link
                </Link>
                <Link
                  className="text-sm font-semibold px-2 py-1 rounded-md underline"
                  to={datum.links.website}
                >
                  Website Link
                </Link>
              </div>
